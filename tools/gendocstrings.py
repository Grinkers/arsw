...
def generate_typestubs(items: list[dict]) -> None:
    ...
    apswtypes = pathlib.Path("src/apswtypes.py").read_text()

    top = apswtypes.split("# Async Note:")[0]

    out.write(top)

    async_out = io.StringIO()

    async_type_aliases = set()

    for line in top.splitlines():
        mo = re.match(r"^(?P<name>\w+)\s*=.*Callable.*$", line)
        if mo:
            name = mo.group("name")
            async_type_aliases.add(name)
    ...
    for item in sorted(items, key=lambda x: x["symbol"]):
        ...
        # this is the least worst way I have found to get mypy happy
        # because they really don't want you defining a singleton, and
        # of course special case None
        if item["name"] == "apsw.no_change":
            print("class no_change:", file=out)
            print('    """', end="", file=out)
            indent = ""
...
{wrapvals(mi["members"])}"""''',
    ...
    print(async_out.getvalue(), file=out)
...
# Awaitable[X] to return types X of Callable.  this is needed to
# indicate that async functions are accepted in addition to sync
# in the many apsw callbacks
def callable_async(sig: str) -> str:
    res, pos = "", 0
    while m := re.search(r"\bCallable\s*\[", sig[pos:]):
        content_idx = pos + m.end()
        res += sig[pos:content_idx]
        ...
        ret = callable_async(sig[last_comma + 1 : i - 1].strip())

        res += f"{args} {ret} | Awaitable[{ret}]"
        pos = i - 1
